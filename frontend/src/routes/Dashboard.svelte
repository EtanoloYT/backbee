...
<div class="flex">
    <aside class="h-screen w-24 border-r-2">
        <div class="flex flex-col items-center justify-between h-full mx-auto py-8">
            <Link to="/" class="flex items-center text-2xl font-semibold text-gray-900">
                <h1 class="text-2xl">BB</h1>
            </Link>
            <nav class="flex flex-col items-center justify-between h-1/3 w-full">
                <Link to="/" class="navlink text-black hover:text-black text-lg border-2 p-1 rounded-lg border-black">
                    <img src="../icons/database.svg" alt="" class="w-8">
                </Link>
                <Link to="/features" class="navlink text-black hover:text-black text-lg">
                    <img src="../icons/analytics.svg" alt="" class="w-8">
                </Link>
                <Link to="/services" class="navlink text-black hover:text-black text-lg">
    ...
    <aside class="w-60 border-r-2 h-screen py-8 flex flex-col">
        <div class="w-will flex items-center justify-center">
            <h2 class="text-lg text-gray-500">Collections</h2>
        </div>
    </aside>
    <main class="w-full h-screen p-8 flex flex-col gap-8">
        ...
        <div class="w-full h-12 flex items-center">
            <input type="text" placeholder="Search Collection" class="w-full h-full rounded-full bg-gray-200 px-8 text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-600">
        </div>
        <div class="w-full relative">
            {#if collection.data.length > 0}
                <table class="relative w-full">
                    ...
                    <tbody class="overflow-scroll w-full">
                        {#each collection.data as item}
                            <tr class="border-b border-gray-200 h-16 hover:bg-gray-100 w-full">
            ...
            {:else}
                <div class="w-full h-1/2 flex flex-col items-center justify-center">
                    ...
                    <button class="px-4 py-2 text-white bg-primary rounded-lg mt-4">Create Collection</button>
                </div>
            {/if}
        </div>
    </main>
